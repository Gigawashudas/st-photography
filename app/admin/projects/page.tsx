...
export default async function AdminProjectsPage() {
  ...
  const { data: projects, error } = await supabase
    .from('projects')
    .select(
      'id, slug, title, category, location, year, cover_image, featured, featured_order, published',
    )
      ...
      <div className="mx-auto max-w-360 px-6 py-8 sm:px-8 sm:py-10 lg:px-10 lg:py-12">
        ...
        <header className="flex items-center justify-between border-b border-foreground/10 pb-8">
          ...
          <Link
            ...
            className="group flex items-center gap-3 border border-foreground/20 px-5 py-3 text-[10px] font-medium uppercase tracking-[0.2em] transition-all duration-300 hover:border-foreground hover:bg-foreground hover:text-background"
          >
            {' '}
        ...
        <section className="pt-20 sm:pt-28 lg:pt-32">
          ...
          <div className="flex flex-col justify-between gap-10 lg:flex-row lg:items-end">
            ...
            <div>
              ...
              <h1 className="font-serif text-[clamp(4rem,9vw,9rem)] leading-[0.8] tracking-tighter">
                {' '}
                Projects.{' '}
              </h1>{' '}
            </div>{' '}
            <p className="max-w-sm text-sm leading-7 text-secondary">
              {' '}
              Manage your interior photography and cinematography portfolio.{' '}
            </p>{' '}
          </div>{' '}
        </section>{' '}
        <section className="mt-20 sm:mt-28">
          ...
          <div className="mb-6 flex items-center justify-between border-b border-foreground/10 pb-5">
            ...
            <p className="hidden text-[10px] uppercase tracking-[0.2em] text-muted sm:block">
              {' '}
              Featured / Status{' '}
            </p>{' '}
          </div>{' '}
          {!projects || projects.length === 0 ? (
            <div className="border border-dashed border-foreground/20 py-24 text-center">
              {' '}
              <p className="font-serif text-4xl tracking-[-0.03em]"> No projects yet. </p>{' '}
              ...
              <Link
                href="/admin/projects/new"
                className="mt-8 inline-flex items-center gap-3 border border-foreground/20 px-6 py-3 text-[10px] font-medium uppercase tracking-[0.2em] transition-all duration-300 hover:border-foreground hover:bg-foreground hover:text-background"
              >
                {' '}
            ...
            <div className="divide-y divide-foreground/10 border-b border-foreground/10">
              ...
              {projects.map((project, index) => (
                <Link
                  ...
                  className="group grid gap-6 py-7 transition-opacity duration-300 hover:opacity-70 sm:grid-cols-[56px_1fr_auto] sm:items-center"
                >
                  {' '}
                  ...
                  <div className="flex min-w-0 items-center gap-5">
                    ...
                    {project.cover_image ? (
                      ...
                      <div className="flex h-16 w-20 shrink-0 items-center justify-center bg-subtle text-[9px] uppercase tracking-[0.15em] text-muted sm:h-20 sm:w-28">
                        {' '}
                        No Image{' '}
                      </div>
                    )}{' '}
                    <div className="min-w-0">
                      {' '}
                      <h2 className="truncate font-serif text-2xl tracking-[-0.02em] sm:text-3xl">
                        {' '}
                        {project.title}{' '}
                      </h2>{' '}
