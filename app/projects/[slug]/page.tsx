    ...
    <main className="min-h-screen bg-background text-foreground transition-colors duration-500">
      ...
      <div className="mx-auto max-w-[1600px] px-6 py-8 sm:px-8 sm:py-10 lg:px-10 lg:py-12">
        ...
        <header className="flex items-center justify-between border-b border-foreground/10 pb-8">
          ...
          <Link href="/" className="group flex items-center gap-3 text-[10px] font-medium uppercase tracking-[0.2em] text-muted transition-colors hover:text-foreground">
          ...
          <section className="border-t border-foreground/10 pt-10 sm:pt-16">
            ...
            <div className="grid gap-4 sm:grid-cols-2 sm:gap-6">
              {galleryImages.map((image, index) => (
                <div key={image} className={`relative overflow-hidden bg-subtle ${index % 3 === 0 ? "sm:col-span-2 aspect-[16/9]" : "aspect-[4/5]"}`}>
                  <Image src={image} alt={`${project.title} — image ${index + 1}`} fill sizes={index % 3 === 0 ? "(max-width: 640px) 100vw, 100vw" : "(max-width: 640px) 100vw, 50vw"} className="object-cover transition-transform duration-700 hover:scale-[1.02]" />
                </div>
              ))}
            </div>
          </section>
        ...
        <section className="border-t border-foreground/10 py-16 sm:py-24 lg:py-32">
          <div className="flex flex-col justify-between gap-10 sm:flex-row sm:items-end">
            <div>
              ...
              <h2 className="mt-5 font-serif text-5xl tracking-[-0.04em] sm:text-7xl">Explore more.</h2>
            </div>

            <Link href="/" className="group flex items-center gap-3 border border-foreground/20 px-6 py-4 text-[10px] font-medium uppercase tracking-[0.2em] transition-all duration-300 hover:border-foreground hover:bg-foreground hover:text-background">
              View portfolio
