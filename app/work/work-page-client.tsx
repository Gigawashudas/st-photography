  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <section className="px-6 pb-24 pt-36 sm:px-8 sm:pb-32 sm:pt-44 lg:px-10 lg:pb-40 lg:pt-52">
        ...
        <div className="mx-auto max-w-360">
          ...
          >
            ...
            <p className="max-w-sm text-sm font-light leading-7 tracking-wider text-secondary sm:text-base sm:leading-8 lg:pb-2">Interior photography and cinematography for spaces, architecture, designers, developers, hospitality, and brands.</p>
          </motion.div>
          ...
          >
            <div className="flex flex-wrap gap-x-7 gap-y-4 sm:gap-x-10">
              {filters.map((filter) => {
                ...
                return (
                  <button key={filter} type="button" onClick={() => setActiveFilter(filter)} className={`relative text-[9px] font-medium uppercase tracking-[0.25em] transition-opacity duration-300 sm:text-[10px] ${isActive ? "text-foreground" : "text-muted hover:text-foreground"}`}>
                    {filter}

                    <span className={`absolute -bottom-2 left-0 h-px bg-foreground transition-all duration-500 ${isActive ? "w-full" : "w-0"}`} />
                  </button>
                );
              })}
            </div>
      ...
      <section className="px-6 pb-32 sm:px-8 sm:pb-40 lg:px-10 lg:pb-52">
        <div className="mx-auto max-w-360">
          <motion.div layout className="grid gap-x-8 gap-y-24 md:grid-cols-2 md:gap-x-10 md:gap-y-36">
            {visibleProjects.map((project, index) => {
              ...
              const aspectClass = isWide ? "aspect-[16/10]" : index % 2 === 0 ? "aspect-[4/5]" : "aspect-[3/4]";
                  ...
                  <Link href={`/projects/${project.slug}`} className="group block">
                    <div className="relative overflow-hidden bg-subtle">
                      <div className={`relative ${aspectClass}`}>
                        <Image src={project.cover_image} alt={project.title} fill sizes={isWide ? "(max-width: 768px) 100vw, 72vw" : "(max-width: 768px) 100vw, 50vw"} className="object-cover transition-transform duration-1200 ease-out group-hover:scale-[1.035]" />
                    ...
                    <div className="mt-5 flex items-start justify-between gap-8">
                      <div>
                        <h2 className="font-serif text-2xl leading-none tracking-tight sm:text-3xl lg:text-4xl">{project.title}</h2>
                      ...
                      <span className="pt-1 text-[10px] font-medium tracking-[0.2em] text-muted sm:text-[11px]">{String(index + 1).padStart(2, "0")}</span>
                    </div>
                  </Link>
                </motion.article>
              );
